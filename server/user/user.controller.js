...
exports.store = async (req, res) => {
  try {
    ...
    if (Number(req.body.loginType) == 0 || Number(req.body.loginType) == 1) {
      ...
      userQuery = await User.findOne({ identity: req.body.identity, loginType: 2 });
      ...
      const emailExist = await User.findOne({ email: req.body.email.trim(), loginType: 3 });
      if (emailExist) {
        ...
        try {
          ...
          return res.status(200).json({
            status: false,
            message: "Failed to decrypt password. Possible corruption or mismatch.",
            error: err.message,
          });
        }
      ...
      const emailExist = await User.findOne({ email: req.body.email.trim(), loginType: 4 });
      if (emailExist) {
        userQuery = emailExist;
      }
